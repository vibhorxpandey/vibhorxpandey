<!-- Profile README for github.com/vibhorxpandey.
     Assets live in /assets. The contribution snake is generated by
     .github/workflows/snake.yml — run it once from the Actions tab after pushing,
     or the snake image below will 404 until the first scheduled run. -->

<div align="center">

<img src="assets/hero.svg" alt="Vibhor Pandey — builder · researcher · founder" width="100%">

<img src="https://readme-typing-svg.demolab.com/?font=JetBrains+Mono&weight=500&size=19&duration=3000&pause=1000&color=E3B341&center=true&vCenter=true&width=680&height=45&lines=Founder+%40+Veil;Building+fact-checked+AI+research+infrastructure;ML+engineer+%E2%80%A2+hybrid+retrieval+%E2%80%A2+agentic+systems;Author+of+The+Quiet+Mathematics+of+Life" alt="Founder @ Veil · fact-checked AI research infrastructure · hybrid retrieval · agentic systems · author of The Quiet Mathematics of Life">
...
I'm a second-year B.Tech CSE (AI/ML) student at UPES, Dehradun, and I got impatient with toy projects early. So I ship production systems instead: **[Veil](https://veilresearch.com)**, the AI-native research-infrastructure company I founded; **[Aurelius](https://github.com/vibhorxpandey/Aurelius)**, a fact-checked research MCP server on PyPI that refuses to show you a citation it hasn't verified; and a series of retrieval systems I ablate before I talk about them — Recall@1 and MRR, or it didn't happen.
...
| **[Veil Finance](https://veilfinance.ink)** | AI research terminal for Indian small/mid-cap equities | portfolio-aware materiality classifier over SEBI LODR Reg-30 labels — LightGBM + Mondrian conformal prediction |
| **[Aurelius](https://github.com/vibhorxpandey/Aurelius)** | fact-checked research MCP server | `v0.6.0` on PyPI — widening verification backends, deepening autonomous mode |
...
A research assistant is only as good as its worst citation. **Aurelius** is a research MCP server that verifies **every citation** against OpenAlex + Crossref — DOI-backed and retraction-aware — and **every claim** against live web sources, *before* anything reaches you. It plugs into any MCP-capable app (Claude Desktop & Claude Code, Gemini CLI, Cursor, ChatGPT) and runs host-driven, so it needs no LLM key of its own. An autonomous mode is there when you want it to run alone.

```text
 topic ──► screen ──► draft ──► fact-check ──► revise ──► you
                                     │
                                     ├── every citation ⇄ OpenAlex + Crossref   (DOI-backed, retraction-aware)
                                     └── every claim    ⇄ live web sources      (verified before it reaches you)
```
...
Research tooling that treats verification as infrastructure, not garnish.
...
<p>A unified AI workspace for researchers and ML engineers — five expert modes and citation-enforced research-paper generation, where compile/export is <b>structurally blocked</b> on unresolved citations. You cannot ship an unverified claim; the compiler won't let you.</p>
...
<p>An AI-native research terminal for Indian small- and mid-cap equity markets. At its core: a portfolio-aware materiality classifier over SEBI LODR Reg-30 disclosure labels — LightGBM under Mondrian conformal prediction, so every signal carries a calibrated confidence guarantee rather than a shrug.</p>
...
<p><i>"The senior who never graduates."</i> An institutional-memory AI agent for college clubs — hybrid retrieval over Qdrant (BM25 + dense + Reciprocal Rank Fusion + cross-encoder reranking) with long-term memory. Verified ablation: <b>93% Recall@1 · 0.967 MRR</b>.</p>
...
<p>AI lead-generation and income assessment for retail lending, built for <b>IDBI Innovate 2026 · Track 02</b>. Uplift-modeled targeting with SHAP explainability and fairness auditing baked in — because a credit model you can't explain is a liability, not an asset.</p>
...
<p>A hybrid-retrieval scoring engine — BM25 + BGE-M3 dense + cross-encoder reranking, with honeypot / prompt-injection detection and <b>76/76 tests green</b> — and the recruiter-screening Lemma pod that wraps it. Built for the Gappy AI Hackathon.</p>
...
- **[remindr](https://github.com/vibhorxpandey/remindr)** — task-reminder PWA with an adaptive neural network and an AI companion named Rem *(TypeScript)*
- **[jarvis / FRIDAY](https://github.com/vibhorxpandey/jarvis)** — speech-recognition voice assistant prototype *(Python)*
...
| | |
|--:|:--|
| **retrieval** | hybrid sparse + dense search — BM25 + BGE-M3 fused with Reciprocal Rank Fusion, cross-encoder reranking on top; Qdrant and pgvector in production; ablated properly (Recall@1, MRR) |
| **llm systems** | RAG pipelines, agentic orchestration, MCP servers, LiteLLM gateways, self-hosted inference (Qwen 14B on vLLM), honeypot &amp; prompt-injection detection, citation-integrity gating |
...
<img src="assets/quiet-mathematics.svg" alt="&quot;Reality has a grammar — entropy, equilibrium, signal, noise. Most of us never learn to read it.&quot; — The Quiet Mathematics of Life, a book in progress" width="100%">
